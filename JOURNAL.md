...
## 2026-06-09 — Le harnais passe au vert : l'étape 3 est close

Pour la première fois, nous avons régénéré les épics et les features depuis le PRD via le Product Manager, dans `conception/specifications/`, puis lancé le harnais Kotlin sur le résultat. Verdict : **« Conforme — aucune violation »**, code de sortie 0. La chaîne complète — PRD → table de dérivation → 4 épics → 11 features → validation — fonctionne de bout en bout, et la conformité est désormais *garantie par un programme*, plus seulement espérée. Le contenu reprend la structure des générations précédentes (4 épics, 11 features, tout en Lot 1), mais le saut est ailleurs : c'est la démonstration vivante de la thèse du projet — des spécifications traitées comme du code, dérivées d'une source unique et validées par des tests. L'étape 3 du harnais est close ; restent l'étape 4 (brancher la boucle « regenerate-until-green » dans le flux) et le chantier d'index par registre.
